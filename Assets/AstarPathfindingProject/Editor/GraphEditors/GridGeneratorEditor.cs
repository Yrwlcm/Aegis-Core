...
namespace Pathfinding {
	...
	public class GridGraphEditor : GraphEditor {
		...
		private GraphTransform savedTransform;
		private Vector2 savedDimensions;
		private float savedNodeSize;
		...
		/// <summary>Cached gui style</summary>
		private static GUIStyle gridPivotSelectBackground;

		/// <summary>Cached gui style</summary>
		private static GUIStyle gridPivotSelectButton;
		...
		private bool IsHexagonal (GridGraph graph) {
		...
		private InspectorGridMode DetermineGridType (GridGraph graph) {
		...
		private void DrawInspectorMode (GridGraph graph) {
		...
		private GUIContent[] hexagonSizeContents = {
		...
		private void DrawFirstSection (GridGraph graph) {
		...
		private void DrawRotationField (GridGraph graph) {
			if (graph.is2D) {
		...
		private void DrawWidthDepthFields (GridGraph graph, out int newWidth, out int newDepth) {
		...
		private void DrawIsometricField (GridGraph graph) {
		...
		private static Vector3 NormalizedPivotPoint (GridGraph graph, GridPivot pivot) {
			switch (pivot) {
		...
		private void DrawPositionField (GridGraph graph) {
			GUILayout.BeginHorizontal();
			var normalizedPivotPoint = NormalizedPivotPoint(graph, pivot);
		...
		private void DrawLastSection (GridGraph graph) {
			GUILayout.BeginHorizontal();
			GUILayout.Space(18);
		...
		private void DrawPenaltyModifications (GridGraph graph) {
		...
		private Vector3[] arcBuffer = new Vector3[21];
		private Vector3[] lineBuffer = new Vector3[2];
		...
		private void DrawDashedLine (Vector2 a, Vector2 b, float dashLength) {
			if (dashLength == 0) {
				DrawLine(a, b);
			} else {
		...
		private static int RoundUpToNextOddNumber (float x) {
			return Mathf.CeilToInt((x - 1)/2.0f)*2 + 1;
		}

		private float interpolatedGridWidthInNodes = -1;
		private float lastTime = 0;

		private void DrawCollisionPreview (GraphCollision collision) {
			EditorGUILayout.BeginHorizontal();
			GUILayout.Space(2);
