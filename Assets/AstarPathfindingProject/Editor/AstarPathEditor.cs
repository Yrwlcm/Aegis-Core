...
namespace Pathfinding {
	...
	public class AstarPathEditor : Editor {
		...
		private static Dictionary<string, CustomGraphEditorAttribute> graphEditorTypes = new Dictionary<string, CustomGraphEditorAttribute>();

		/// <summary>
		/// Holds node counts for each graph to avoid calculating it every frame.
		/// Only used for visualization purposes
		/// </summary>
		private static Dictionary<NavGraph, KeyValuePair<float, KeyValuePair<int, int> > > graphNodeCounts;

		/// <summary>List of all graph editors for the graphs</summary>
		private GraphEditor[] graphEditors;

		private System.Type[] graphTypes {
			get {
				return script.data.graphTypes;
			}
		}

		private static int lastUndoGroup = -1000;

		/// <summary>Used to make sure correct behaviour when handling undos</summary>
		private static uint ignoredChecksum;

		private const string scriptsFolder = "Assets/AstarPathfindingProject";

		#region SectionFlags

		private static bool showSettings;
		private static bool customAreaColorsOpen;
		private static bool editTags;

		private FadeArea settingsArea;
		private FadeArea colorSettingsArea;
		private FadeArea editorSettingsArea;
		private FadeArea aboutArea;
		private FadeArea optimizationSettingsArea;
		private FadeArea serializationSettingsArea;
		private FadeArea tagsArea;
		private FadeArea graphsArea;
		private FadeArea addGraphsArea;
		private FadeArea alwaysVisibleArea;
		...
		private static GUIStyle level0AreaStyle, level0LabelStyle;
		private static GUIStyle level1AreaStyle, level1LabelStyle;

		private static GUIStyle graphDeleteButtonStyle, graphInfoButtonStyle, graphGizmoButtonStyle, graphEditNameButtonStyle;
		...
		private static bool IsJsEnabled () {
		...
		private GraphEditor graphNameFocused;

		private void DrawGraphHeader (GraphEditor graphEditor) {
		...
		private void DrawGraphInfoArea (GraphEditor graphEditor) {
		...
		private void DrawGraph (GraphEditor graphEditor) {
		...
		private void DrawSceneGUISettings () {
		...
		private TextAsset SaveGraphData (byte[] bytes, TextAsset target = null) {
		...
		private void DrawHeuristicOptimizationSettings () {
			// Pro only feature
		}
		...
		private static void DrawColorSlider (ref float left, ref float right, bool editable) {
			GUILayout.BeginHorizontal();
			GUILayout.Space(20);
		...
		private void CheckGraphEditors (bool forceRebuild = false) {
		...
		private void RemoveGraph (NavGraph graph) {
		...
		private void AddGraph (System.Type type) {
		...
		private GraphEditor CreateGraphEditor (NavGraph graph) {
		...
		private static int ByteArrayHash (byte[] arr) {
		...
		private void SerializeIfDataChanged () {
			uint checksum;
		...
		private void DeserializeGraphs (byte[] bytes) {
			try {
